...
#----Note that this code assumes the denormalized input is NON-ZERO.
	...
	mov.l		&0x00000000,%d3	# D3 is exponent of smallest norm. #
	mov.l		4(%a0),%d4
...
#--A FINITE, NON-ZERO, NORMALIZED NUMBER.
	...
	cmp.l		%d0,&0		# CHECK IF X IS NEGATIVE
...
#--WHILE THE CONVERSION IS GOING ON, WE GET F AND ADDRESS OF 1/F
	...
	mov.l		FFRAC(%a6),%d0	# READY TO GET ADDRESS OF 1/F
	and.l		&0x7E000000,%d0
	asr.l		&8,%d0
	asr.l		&8,%d0
	asr.l		&4,%d0		# SHIFTED 20, D0 IS THE DISPLACEMENT
	add.l		%d0,%a0		# A0 IS THE ADDRESS FOR 1/F

	fmov.x		X(%a6),%fp0
	mov.l		&0x3fff0000,F(%a6)
	clr.l		F+8(%a6)
	fsub.x		F(%a6),%fp0	# Y-F
	fmovm.x		&0xc,-(%sp)	# SAVE FP2 WHILE FP0 IS NOT READY {%fp2/%fp3}
...
#--[U + V*(A1+V*(A3+V*A5))]  +  [U*V*(A2+V*(A4+V*A6))]
	...
	fmovm.x		(%sp)+,&0x30	# RESTORE FP2 {%fp2/%fp3}
...
#--ENTRY POINT FOR LOG(1+X) FOR X FINITE, NON-ZERO, NOT NAN'S
	...
	fmov.l		&0,%status	# clr N flag from compare
	fmov.l		%d1,%control
...
#--(1/F) IN A0, Y-F IN FP0, AND FP2 SAVED.
	...
	cmp.l		%d0,&0x3FFF8000	# SEE IF 1+Z > 1
	bge.b		KISZERO
...
KISZERO:
	...
	lea		LOGTBL,%a0
	add.l		%d0,%a0		# A0 IS ADDRESS OF 1/F
	fmov.s		zero,%fp1	# FP1 IS K = 0
	bra.w		LP1CONT1
