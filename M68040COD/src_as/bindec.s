...
#
A2_str:
	mov.l		(%a0),FP_SCR2(%a6)	# move input to work space
	mov.l		4(%a0),FP_SCR2+4(%a6)	# move input to work space
	mov.l		8(%a0),FP_SCR2+8(%a6)	# move input to work space
	and.l		&0x7fffffff,FP_SCR2(%a6)	# create abs(X)
...
#	L_SCR2:first word of X packed/Unchanged
	...
	bra.b		A4_str
A3_cont:
	mov.w		FP_SCR2(%a6),%d0	# move exp to d0
	mov.w		&0x3fff,FP_SCR2(%a6)	# replace exponent with 0x3fff
	fmov.x		FP_SCR2(%a6),%fp0	# now fp0 has 1.f
...
A4_str:
	fmov.l		&0,%status	# zero all of fpsr - nothing needed
...
x_pos:
	...
	fmov.l		%d3,%control	# load bits into fpu
...
#	Input/Output
	...
	fmov.l		&0,%status	# clr INEX 
	fmov.l		&rz_mode,%control	# set RZ rounding mode
...
sc_mul:
	...
	fmovm.x		&0x2,-(%sp)	# load ETEMP with 10^ISCALE {%fp1}
	mov.l		8(%a0),-(%sp)	# load FPTEMP with input arg
	mov.l		4(%a0),-(%sp)
	mov.l		(%a0),-(%sp)
	mov.l		&18,%d3		# load count for busy stack
A9_loop:
	clr.l		-(%sp)		# clear lword on stack
	dbf.w		%d3,A9_loop
	mov.b		VER_TMP(%a6),(%sp)	# write current version number
	mov.b		&BUSY_SIZE-4,1(%sp)	# write current busy size 
	mov.b		&0x10,0x44(%sp)	# set fcefpte[15] bit
	mov.w		&0x0023,0x40(%sp)	# load cmdreg1b with mul command
	mov.b		&0xfe,0x8(%sp)	# load all 1s to cu savepc
	frestore	(%sp)+		# restore frame to fpu for completion
...
A10_st:
	fmov.l		%status,%d0	# get FPSR
	fmov.x		%fp0,FP_SCR2(%a6)	# move Y to memory
	...
	fmov.x		FP_SCR2(%a6),%fp0	# write adjusted Y back to fpu
...
A12_st:
	...
	fmov.x		%fp0,(%a0)	# move Y to memory at FP_SCR2(a6)
...
do_fint:
	...
	mov.l		L_SCR2(%a6),FP_SCR2(%a6)	# restore original exponent
...
# Compute 10^(LEN-1)
#
	fmov.s		FONE,%fp2	# init fp2 to 1.0
...
A13_con:
	...
	mov.w		&1,%d5		# set ICTR
	fmov.l		&rm_mode,%control	# set rmode to RM
...
A14_st:
	fmov.l		&rz_mode,%control	# force rz for conversion
...
den_zero:
	...
	fmov.s		F4933,%fp0	# force exponent to 4933
	bra.b		convrt		# do it
use_ilog:
	fmov.l		%d6,%fp0	# float ILOG
	fabs.x		%fp0		# get abs of ILOG
...
not_denorm:
	...
	fmov.s		FONE,%fp0	# force exponent to 1
	bra.b		convrt		# do it
not_zero:
	fmov.l		%d6,%fp0	# float ILOG
	fabs.x		%fp0		# get abs of ILOG
convrt:
	...
	fmov.x		%fp0,FP_SCR2(%a6)	# store fp0 in memory
...
A16_st:
	...
	and.b		&0x0f,FP_SCR1(%a6)	# clear first nibble of FP_SCR1 
...
# Clean up and restore all registers used.
	...
	fmov.l		&0,%status	# clear possible inex2/ainex bits
