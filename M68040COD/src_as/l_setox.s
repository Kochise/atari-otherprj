...
#--entry point for EXP(X), X is denormalized
	mov.l		(%a0),%d0
	and.l		&0x80000000,%d0
	or.l		&0x00800000,%d0	# sign(X)*2^(-126)
	mov.l		%d0,-(%sp)
...
#--Step 1.
	mov.l		(%a0),%d0	# load part of input X
	and.l		&0x7FFF0000,%d0	# biased expo. of X
	cmp.l		%d0,&0x3FBE0000	# 2^(-65)
	bge.b		EXPC1		# normal case
	bra.w		EXPSM

EXPC1:
#--The case |X| >= 2^(-65)
	mov.w		4(%a0),%d0	# expo. and partial sig. of |X|
	cmp.l		%d0,&0x400CB167	# 16380 log2 trunc. 16 bits
...
#--This is the normal branch:	2^(-65) <= |X| < 16380 log2.
	...
	mov.l		&0,ADJFLAG(%a6)
	fmov.l		%fp0,%d0	# N = int( X * 64/log2 )
	lea		EXPTBL,%a1
	fmov.l		%d0,%fp0	# convert to floating-format

	mov.l		%d0,L_SCR1(%a6)	# save N temporarily
	and.l		&0x3F,%d0	# D0 is J = N mod 64
	lsl.l		&4,%d0
	add.l		%d0,%a1		# address of 2^(J/64)
	mov.l		L_SCR1(%a6),%d0
	asr.l		&6,%d0		# D0 is M
	add.w		&0x3FFF,%d0	# biased expo. of 2^(M)
	mov.w		L2,L_SCR1(%a6)	# prefetch L2, no need in CB
...
#	MOVE.W		#0,2(a1)	...load 2^(J/64) in cache
	...
	mov.w		%d0,SCALE(%a6)	# SCALE is 2^(M) in extended
	clr.w		SCALE+2(%a6)
...
NORMAL:
	fmov.l		%d1,%control	# restore user FPCR
	fmul.x		SCALE(%a6),%fp0	# multiply 2^(M)
	bra		t_frcinx

EXPSM:
#--Step 7
	fmovm.x		(%a0),&0x80	# in case X is denormalized {%fp0}
	fmov.l		%d1,%control
...
EXPBIG:
#--Step 8
	cmp.l		%d0,&0x400CB27C	# 16480 log2
	bgt.b		EXP2BIG
#--Steps 8.2 -- 8.6
	fmov.x		(%a0),%fp0	# load input from (a0)
	...
	mov.l		L_SCR1(%a6),%d0
	asr.l		&6,%d0		# D0 is K
	mov.l		%d0,L_SCR1(%a6)	# save K temporarily
	asr.l		&1,%d0		# D0 is M1
	sub.l		%d0,L_SCR1(%a6)	# a1 is M
	add.w		&0x3FFF,%d0	# biased expo. of 2^(M1)
	mov.w		%d0,ADJSCALE(%a6)	# ADJSCALE := 2^(M1)
	clr.w		ADJSCALE+2(%a6)
	mov.l		&0x80000000,ADJSCALE+4(%a6)
	clr.l		ADJSCALE+8(%a6)
	mov.l		L_SCR1(%a6),%d0	# D0 is M
	add.w		&0x3FFF,%d0	# biased expo. of 2^(M)
...
#--The case |X| >= 1/4
	mov.w		4(%a0),%d0	# expo. and partial sig. of |X|
	cmp.l		%d0,&0x4004C215	# 70log2 rounded up to 16 bits
...
#	MOVE.W		#$3F81,EM1A4		...prefetch in CB mode
	...
	mov.l		L_SCR1(%a6),%d0
	asr.l		&6,%d0		# D0 is M
	mov.l		%d0,L_SCR1(%a6)	# save a copy of M
...
#	MOVE.W		#$3FC5,EM1A2		...load EM1A2 in cache
	add.w		&0x3FFF,%d0	# D0 is biased expo. of 2^M
...
#	MOVE.W		#0,2(a1)	...load 2^(J/64) in cache
	...
	mov.w		%d0,SC(%a6)	# SC is 2^(M) in extended
	clr.w		SC+2(%a6)
	...
	or.w		&0x8000,%d0	# signed/expo. of -2^(-M)
	mov.w		%d0,ONEBYSC(%a6)	# OnebySc is -2^(-M)
	clr.w		ONEBYSC+2(%a6)
