...
#--ENTRY POINT FOR 2**(X) FOR DENORMALIZED ARGUMENT
	...
	fmov.l		%d1,%control	# set user's rounding mode/precision
...
#--ENTRY POINT FOR 2**(X), HERE X IS FINITE, NON-ZERO, AND NOT NAN'S
	fmovm.x		(%a0),&0x80	# LOAD INPUT, do not set cc's {%fp0}
	...
	cmp.l		%d0,&0x3FB98000	# |X| >= 2**(-70)?
	bge.b		TWOOK1
	bra.w		EXPBORS

TWOOK1:
	cmp.l		%d0,&0x400D80C0	# |X| > 16480?
	ble.b		TWOMAIN
	bra.w		EXPBORS
...
#--USUAL CASE, 2^(-70) <= |X| <= 16480
	...
	fmov.l		%fp1,N(%a6)	# N = ROUND-TO-INT(64 X)
	mov.l		%d2,-(%sp)
	lea		EXPTBL,%a1	# LOAD ADDRESS OF TABLE OF 2^(J/64)
	fmov.l		N(%a6),%fp1	# N --> FLOATING FMT
	mov.l		N(%a6),%d0
	mov.l		%d0,%d2
	and.l		&0x3F,%d0	# D0 IS J
	asl.l		&4,%d0		# DISPLACEMENT FOR 2^(J/64)
	add.l		%d0,%a1		# ADDRESS FOR 2^(J/64)
	asr.l		&6,%d2		# d2 IS L, N = 64L + J
	mov.l		%d2,%d0
	asr.l		&1,%d0		# D0 IS M
	sub.l		%d0,%d2		# d2 IS M', N = 64(M+M') + J
	add.l		&0x3FFF,%d2
	mov.w		%d2,ADJFACT(%a6)	# ADJFACT IS 2^(M')
	mov.l		(%sp)+,%d2
...
TENOK1:
	cmp.l		%d0,&0x400B9B07	# |X| <= 16480*log2/log10 ?
	ble.b		TENMAIN
	bra.w		EXPBORS
...
#--USUAL CASE, 2^(-70) <= |X| <= 16480 LOG 2 / LOG 10
	...
	fmov.l		%fp1,N(%a6)	# N=INT(X*64*LOG10/LOG2)
	mov.l		%d2,-(%sp)
	lea		EXPTBL,%a1	# LOAD ADDRESS OF TABLE OF 2^(J/64)
	fmov.l		N(%a6),%fp1	# N --> FLOATING FMT
	mov.l		N(%a6),%d0
...
#--	2**(M'+M) * 2**(J/64) * EXP(R)
	...
	fmov.d		EXPA5,%fp2	# FP2 IS A5
	fmov.d		EXPA4,%fp3	# FP3 IS A4
