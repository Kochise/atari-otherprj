...
bug1384:
	link		%a5,&0
	fsave		-(%sp)
	cmp.b		(%sp),&0x41	# check for correct frame
	beq		frame_41
...
idle40:
	...
	mov.l		%d1,USER_D1(%a6)	# save d1
	mov.w		&8,%d1		# place unimp frame instead
loop40:
	clr.l		-(%sp)
	dbra		%d1,loop40
	mov.l		USER_D1(%a6),%d1	# restore d1
	mov.l		&0x40280000,-(%sp)
	frestore	(%sp)+
	unlk		%a5
	rts
...
idle41:
	clr.l		(%sp)		# get rid of old fsave frame
	mov.l		%d1,USER_D1(%a6)	# save d1
	mov.w		&10,%d1		# place unimp frame instead
loop41:
	clr.l		-(%sp)
	dbra		%d1,loop41
	mov.l		USER_D1(%a6),%d1	# restore d1
