...
static void new_fontpoint( GUIDE *tg, int font, int point )
{
  ...
  set_fontpoint( font, point );	*(long *)&tg->font = *(long *)&par.font;
...
static int put_effect( int effect, int new, FBUF *fp )
{
  if (effect >= 0)
  {
    ...
    {
      Fputc( 18, fp ); Fputc( effect & TXT_THICKENED ? '0' : '1', fp );
    }
    if (new & TXT_LIGHT)
    {
      Fputc( 18, fp ); Fputc( effect & TXT_LIGHT ? 'E' : 'F', fp );
    }
    if (new & TXT_SKEWED)
    {
      Fputc( 18, fp ); Fputc( effect & TXT_SKEWED ? '2' : '3', fp );
    }
    if (new & TXT_UNDERLINED)
    {
      Fputc( 18, fp ); Fputc( effect & TXT_UNDERLINED ? '4' : '5', fp );
    }
    if (new & TXT_SUPERSCRIPT)
    {
      Fputc( 18, fp ); Fputc( effect & TXT_SUPERSCRIPT ? '6' : '7', fp );
    }
    if (new & TXT_SUBSCRIPT)
    {
      Fputc( 18, fp ); Fputc( effect & TXT_SUBSCRIPT ? '8' : '9', fp );
  } }
  return effect;
}
  ...
  for (;;)
    switch (i = (unsigned char)*p++)
    {
      case   0: Fputc( 13, &f ); Fputc( 10, &f );
		...
		break;
      case Esc: effect = put_effect( effect, *p++ & 0x3F, &f ); break;
...
static void write_dump( GUIDE *tg )
{
  ...
  f.handle = fh; Fbufcreate( &f ); p = tg->buf; end = tg->text; count = 0;
  do
  { b = 7; val = count;
    do
...
static void rect_invert( int x1, int y1, int x2, int y2 )
{
  ...
  vswr_mode( handle, MD_XOR ); vsf_color( handle, 1 );
  vr_recfl( handle, pxy );
  vsf_color( handle, 0 ); vswr_mode( handle, MD_REPLACE );
}
...
static void draw_text( GUIDE *tg, int *clip )
{
  ...
  if (tg->lc < 0)
  {
    ...
    if (clip[3] > flag) if ((clip[3] = flag) < y) return;
  }
  vs_clip( handle, 1, clip ); mydiv( y - tg->y, tg->yfac, (div_t *)clip );
  tg->line += flag = tg->tlc - tg->line + ((div_t *)clip)->quot;
  y -= ((div_t *)clip)->rem; text = tg->text;
  if (tg->flag == 0)
  {
    char c, e = Esc;
  ...
  {
    if (flag < 0) do { while (*--text); } while (++flag < 0);
    if (flag > 0) do { while (*++text); } while (--flag > 0);
  }
...
static void draw_link( GUIDE *tg )
{
  ...
  tg->line += flag = tg->row - tg->line; text = tg->text;
  if (tg->flag == 0)
  {
    char c, e = Esc;
...
static void set_newlink( GUIDE *tg, char *link, int row )
{
  ...
  if (row - tg->tlc < 0 || row - tg->tlc >= tg->h / tg->yfac)
    goto_line( row - ((tg->h / tg->yfac) >> 1) );
  if (newlink == tg->link) return;
  graf_mouse( M_OFF, 0 );
  vswr_mode( handle, MD_XOR ); vsf_color( handle, 1 );
  if (tg->row - tg->tlc >= 0 && tg->row - tg->tlc < tg->h / tg->yfac)
    draw_link( tg );
  ...
  switch (code)
  {
    ...
    case ENTER: if (tg->link <= 0) return 1;
		...
		open_window( 1, code == HELP, 0, 0, 0, 0,
			     (char *)tg + tg->link + 1, key + 1 );
		((char *)tg)[tg->link] = *end = LINK; return 0;
  }
...
static int handle_link( GUIDE *tg, int mx, int my, int ks )
{
  ...
  flag = tg->tlc - tg->line; while ((y += hchar) <= my) ++flag;
  tg->line += flag; text = tg->text;
  if (tg->flag == 0)
  {
    char c;
...
static void dclick_text( GUIDE *tg, int mx, int my, int ks )
{
  if (ks < 0)
  {
    if (ks == -1) new_fontpoint( tg, mx, my ); else par.hwin = tg->h;
    return;
  }
...
GUIDE *load_text( int fh, long len )
{
  ...
  *(long *)&rg->font = *(long *)&par.font;
  ...
  rg->w = (rg->bc + 1) * wchar; rg->h = par.hwin;
...
GUIDE *load_dump( int fh, long len )
{
  ...
  if ((rg = Malloc( sizeof(GUIDE) + len )) == 0)
    if ((len = Mavail()-sizeof(GUIDE)) > 0) rg = Malloc(sizeof(GUIDE)+len);
  ...
  if (par.textdef) AVSendMessage( AV_ASKFILEFONT, 0, 0 );
  *(long *)&rg->font = *(long *)&par.font;
  rg->yfac = hchar; rg->xfac = wchar; rg->dist = dist;
  rg->h = par.hwin; rg->w = 77 * wchar;
