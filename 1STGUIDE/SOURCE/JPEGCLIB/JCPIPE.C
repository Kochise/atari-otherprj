...
/* On entry, cinfo->comps_in_scan and cinfo->cur_comp_info[] are set up */
{
  ...
  for (ci = 0; (short)ci < cinfo->comps_in_scan; ci++) {
    ...
    if ((mcublks -= compptr->MCU_blocks) < 0)
      ERREXIT(cinfo->emethods, "Sampling factors too large for interleaved scan");
  }

  cinfo->blocks_in_MCU = MAX_BLOCKS_IN_MCU - mcublks;
...
#if DCTSIZE == 8
  cinfo->MCUs_per_row = (compptr->true_comp_width + 7) >> 3;
  cinfo->MCU_rows_in_scan = (compptr->true_comp_height + 7) >> 3;
#else
  cinfo->MCUs_per_row = (compptr->true_comp_width + DCTSIZE-1) / DCTSIZE;
  cinfo->MCU_rows_in_scan = (compptr->true_comp_height + DCTSIZE-1) / DCTSIZE;
#endif

  compptr->downsampled_width = cinfo->MCUs_per_row * DCTSIZE;
...
/* (see comments at head of file) */
{
  ...
  fullsize_data[0] = (JSAMPIMAGE) (*cinfo->emethods->alloc_small)
				(cinfo->input_components * (2 * SIZEOF(JSAMPARRAY)));
  fullsize_data[1] = fullsize_data[0] + cinfo->input_components;
...
/* Release a sampling buffer created by alloc_sampling_buffer */
{
  ...
  for (ci = 0; (short)ci < cinfo->input_components; ci++) {
	     ...
	     JSAMPIMAGE image_data)
{
  ...
  if (input_cols < output_cols) {
	...
	ptr = *this_component++ + input_cols - 1;
	pixval = *ptr++;		/* don't need GETJSAMPLE() here */
	for (col = input_cols; col < output_cols; col++)
	  *ptr++ = pixval;
      }
    }
  }
  ...
  if (input_rows < output_rows) {
	...
	jcopy_sample_rows(this_component, row - 1,
			  this_component, row,
			  1, output_cols);
      }
    }
...
/* Special case: above, below can be -1 to indicate top, bottom of image. */
{
  ...
  for (ci = 0; (short)ci < cinfo->num_components; ci++) {
    compptr = & cinfo->comp_info[ci];

    if (above >= 0)
      above_ptr = fullsize_data[0] + above * vs;
    else {
      /* Top of image: make a dummy above-context with copies of 1st row */
      /* We assume current=0 in this case */
      for (i = 0; i < vs; i++)
	dummy[i] = fullsize_data[0][0];
      above_ptr = dummy;
    }

    if (below >= 0)
      below_ptr = fullsize_data[0] + below * vs;
    else {
      /* Bot of image: make a dummy below-context with copies of last row */
      for (i = 0; i < vs; i++)
	dummy[i] = fullsize_data[0][(current+1)*vs-1];
      below_ptr = dummy;
    }
    ...
    (*cinfo->methods->downsample[ci])
		(cinfo,
		 ...
		 above_ptr,
		 *fullsize_data++ + current * vs,
		 below_ptr,
		 *sampled_data++ + out * compptr->v_samp_factor);
  }
}
...
{
  long rows_in_mem;		/* # of sample rows in full-size buffers */
  ...
  JSAMPARRAY sampled_data[MAX_COMPS_IN_SCAN];
  long rows_this_time, ci;
  short whichss, i;
  ...
  (*cinfo->emethods->alloc_big_arrays)
	(0,				/* no more small sarrays */
	 0,				/* no more small barrays */
	 0);				/* no more "medium" objects */
  ...
  for (cur_pixel_row = 0; cur_pixel_row < cinfo->image_height;
    ...
    (*cinfo->methods->get_pixel_rows) (cinfo, (int) rows_this_time,
				       fullsize_data[whichss]);
    (*cinfo->methods->color_convert) (cinfo, (int) rows_this_time,
				      fullsize_data[whichss]);
    edge_expand (cinfo, cinfo->image_width, (int) rows_this_time,
		 fullsize_width, (int) rows_in_mem, fullsize_data[whichss]);
    ...
    do downsample(cinfo, fullsize_data[whichss], sampled_data, fullsize_width,
		  i-1, i, i+1, i);
    while (++i <= DCTSIZE-2);
  } /* end of outer loop */
  ...
  downsample(cinfo, fullsize_data[whichss], sampled_data, fullsize_width,
	     DCTSIZE-2, DCTSIZE-1, -1, DCTSIZE-1);
  /* Dump the remaining data (may be less than full height if uninterleaved) */
  (*cinfo->methods->extract_MCUs) (cinfo, sampled_data, mcu_rows_output,
