...
static OBJECT popup[] =
{
 ...
 10, -1, -1, G_BUTTON, NONE,   NORMAL,   dmastring,		10,5, 9, 1,
...
static int key_sample( GUIDE *tg, int code, int ks )
{
  switch (code)
  {
    ...
    case CNTRL_M:  if (dma_avail & tg->st_ram)
...
static void sclick_sample( GUIDE *tg, int mx, int my )
{
  ...
  *normstring = tg->dma ? ' ' : 8; *dmastring = tg->dma ? 8 : ' ';
  popup[9].ob_state = dma_avail & tg->st_ram ? NORMAL : DISABLED;
...
GUIDE *load_sample( int fh, long len )
{
  ...
  rg = Mxalloc( sizeof(GUIDE) + len, 0 );
  if (32 + (long)rg == 0)
  {
    rg = Malloc( sizeof(GUIDE) + len );
    if (rg) rg->st_ram = 1;
    else { Fclose( fh ); form_error( EINVMEM ); return 0; }
  }
  else if (rg) rg->st_ram = 1;
       else
       {
	 rg = Mxalloc( sizeof(GUIDE) + len, 1 );
	 if (rg) rg->st_ram = 0;
	 else { Fclose( fh ); form_error( EINVMEM ); return 0; }
       }
...
#endif
  {
    (unsigned)rg->hertz = 10U * ((unsigned *)start)[10];
    stop = start += 50;
  }
#ifdef __TOS__
...
#endif
  {
    (unsigned)rg->hertz = 13000; stop = start += 34;
  }
#ifdef __TOS__
  else if (*(long *)start == '2BIT')		/* MusiCom Format */
#else
  else if (*(long *)start == 'TIB2')
#endif
  {
    if (*(int *)(start + 12) || (*(int *)(start + 14) -= 8))
      { Mfree( rg ); form_error( ENOENT ); return 0; }
    (unsigned)rg->hertz = ((unsigned *)start)[12];
    start += 128; if (*(int *)(start - 112)) stop = start;
  }
#ifdef __TOS__
...
#endif
  {
    (unsigned)rg->hertz = 1000U * ((unsigned char *)start)[3];
    start += 4;
  }
  { char c = 128; while (stop > start) *--stop -= c; }
