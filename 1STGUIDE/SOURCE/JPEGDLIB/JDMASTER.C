...
/* Central point for initial method selection (after reading file header) */
{
  ...
  cinfo->d_per_scan_method_selection = d_per_scan_method_selection;
}
...
jpeg_decompress (decompress_info_ptr cinfo)
{
   ...
   */
  (*cinfo->read_file_header) (cinfo);
  if (! ((*cinfo->read_scan_header) (cinfo)))
  ...
  (*cinfo->d_ui_method_selection) (cinfo);
  ...
  (*cinfo->output_init) (cinfo);
  (*cinfo->colorout_init) (cinfo);
  if (cinfo->quantize_colors)
    (*cinfo->color_quant_init) (cinfo);

  /* And let the pipeline controller do the rest. */
  (*cinfo->d_pipeline_controller) (cinfo);

  /* Finish output file, release working storage, etc */
  if (cinfo->quantize_colors)
    (*cinfo->color_quant_term) (cinfo);
  (*cinfo->colorout_term) (cinfo);
  (*cinfo->output_term) (cinfo);
  (*cinfo->read_file_trailer) (cinfo);
